...
trait KafkaStreamConfiguration {
  ...
  def streamingConfig: Properties = {
    ...
    settings.put(StreamsConfig.REPLICATION_FACTOR_CONFIG, replicationFactor: java.lang.Integer)
    settings.put(StreamsConfig.producerPrefix(ProducerConfig.ACKS_CONFIG), producerMessagesAcknowledge)
    settings.put(StreamsConfig.CACHE_MAX_BYTES_BUFFERING_CONFIG, cacheSizeInBytes) // disable caching
    ...
    settings.put(StreamsConfig.ROCKSDB_CONFIG_SETTER_CLASS_CONFIG, classOf[CustomRocksDBConfig])
    settings.put(ConsumerConfig.MAX_POLL_INTERVAL_MS_CONFIG, maxPollMs)
    settings.put(StreamsConfig.STATE_DIR_CONFIG, s"$basePath/$listenPort/stores")
...
trait IotKafkaStreamConfiguration extends KafkaStreamConfiguration {
  ...
  override val enableConfluentInterceptors: Boolean = config.enableConfluentInterceptors
